...
protected:
  ...
  /// @brief Active device qubits per jobID/taskID, populated only when mapping
  /// ran. Includes unmeasured ancillary qubits and mapper-introduced wires, so
  /// it does
  /// not fit the per-result output_names channel. Consumed by native-mapping
  /// remote backends to build hardware payloads.
  std::map<std::string, ActiveDeviceQubits> activeDeviceQubits;
  ...
  /// @brief Reconstruct a sample_result for jobId from a flat-bitstring counts
  ...
  tryReconstructFromLayout(const std::string &jobId,
