...
ServerHelper::resultMapForJob(const std::string &jobId) const {
  ...
  for (const auto &[result, info] : it->second)
    resultMap.outputs.push_back(
        {info.qubitNum, info.registerName, info.outputPosition});
  return resultMap;
}

std::optional<cudaq::sample_result>
ServerHelper::tryReconstructFromLayout(const std::string &jobId,
                                       const CountsDictionary &counts) {
  auto resultMap = resultMapForJob(jobId);
  if (!resultMap)
    return std::nullopt;
  return reconstructSampleResultFromLocalMeasurements(counts, *resultMap);
}
...
void ServerHelper::parseConfigForCommonParams(
  ...
  for (auto &[key, val] : backendConfiguration) {
    ...
    } else if (key.starts_with("active_device_qubits.")) {
      this->activeDeviceQubits[newKey] =
          nlohmann::json::parse(val).get<ActiveDeviceQubits>();
