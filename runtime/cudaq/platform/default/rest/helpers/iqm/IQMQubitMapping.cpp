...
nlohmann::json cudaq::buildIqmQubitMapping(
    const ActiveDeviceQubits &activeDeviceQubits,
  ...
  if (!activeDeviceQubits.empty()) {
    ...
    // mapping covers exactly the active device qubits produced by the mapper;
    // the full calibrated-qubit set is not required. Resolve each device qubit
    // to its backend label through the ServerHelper base table, which the
    // dynamic architecture fetch populated.
    for (std::size_t localIndex = 0; localIndex < activeDeviceQubits.size();
         ++localIndex) {
      auto deviceQubit = activeDeviceQubits[localIndex];
      auto physicalName = backendLabel(deviceQubit);
      if (!physicalName)
        throw std::runtime_error("mapped IQM device qubit " +
                                 std::to_string(deviceQubit) +
                                 " has no backend physical name");
      nlohmann::json singleQubitMapping;
      singleQubitMapping["logical_name"] =
          "QB" + std::to_string(localIndex + 1);
      singleQubitMapping["physical_name"] = *physicalName;
