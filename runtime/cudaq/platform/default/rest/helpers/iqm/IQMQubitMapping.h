...
/// execution. When `activeDeviceQubits` is non-empty, emit a partial mapping
/// that resolves each active device qubit to its backend label through
/// `backendLabel`, throwing when a device qubit has no label. Otherwise emit
/// the full mapping derived from the dynamic quantum architecture in
/// `qubitNameMap`.
