...
void IQMServerHelper::initialize(BackendConfig config) {
  ...
  // bitstrings through typed execution layouts.
...
IQMServerHelper::createJob(std::vector<KernelExecution> &circuitCodes) {
  ...
  for (auto &circuitCode : circuitCodes) {
    ...
    // createJob runs on the submit side, before the backend assigns a job id,
    // so the parsed active_device_qubits.<task> sidecar
    // (ServerHelper::activeDeviceQubits) is not yet keyed for this execution.
    // The per-execution activeDeviceQubits sidecar carried in memory is the
    // source of the active qubit list at submit time. The serialized sidecar is
    // the transport that lets a re-instantiated helper recover the same fact on
    // the result side. Both carry identical data, so the produced payload
    // matches the sidecar-fed one.
    message["qubit_mapping"] =
        buildIqmQubitMapping(circuitCode.activeDeviceQubits, qubitNameMap,
...
IQMServerHelper::processResults(ServerMessage &postJobResponse,
  ...
  if (auto result = tryReconstructFromLayout(jobID, sampleResult.to_map()))
    return *result;
