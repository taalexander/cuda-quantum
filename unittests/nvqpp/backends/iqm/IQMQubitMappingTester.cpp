...
// buildIqmQubitMapping has two production inputs. When the mapper produced
// active device qubits for an emitted execution, the mapping is the active-list
// composed with the base label table (logical_name is the 1-based local index;
// physical_name is resolved from the label table by device qubit). When there
// are no active device qubits, the full dynamic-architecture name map is used
// directly (logical_name is the 1-based dense index; physical_name is the
// provider qubit name).
TEST(IQMQubitMappingTester, BuildsMappingFromActiveQubitsOrFullNameMap) {
  ...
  struct Case {
    std::string name;
    cudaq::ActiveDeviceQubits activeDeviceQubits;
  ...
  std::vector<Case> cases = {
      {"partialFromActiveAndLabelTable",
       {3, 1},
       emptyNameMap,
       labelTable,
       {{"QB1", "QB4"}, {"QB2", "QB2"}}},
  ...
  for (const auto &testCase : cases) {
    ...
    auto qubitMapping = cudaq::buildIqmQubitMapping(
        testCase.activeDeviceQubits, testCase.nameMap, testCase.backendLabel);
