...
TEST(KernelExecutionTester, AssignmentCopiesActiveDeviceQubitsIndependently) {
  ...
  rhs.targetQubitMapping = {{"QB4", 3}};

  lhs = rhs;
  rhs.activeDeviceQubits[0] = 11;
  rhs.targetQubitMapping[0].deviceQubit = 11;
  EXPECT_EQ(lhs.activeDeviceQubits, (cudaq::ActiveDeviceQubits{3}));
  ASSERT_EQ(lhs.targetQubitMapping.size(), 1);
  EXPECT_EQ(lhs.targetQubitMapping[0].logicalName, "QB4");
  EXPECT_EQ(lhs.targetQubitMapping[0].deviceQubit, 3);
}
