...
``--mlir-print-ir-before`` or ``--mlir-print-ir-after`` to examine the IR around
a transformation. ``cudaq-opt --help`` lists the passes and pipelines available
in the current build. The :doc:`available pass catalog <available_passes>`
documents CUDA-Q's built-in passes and their options.
...
For a quantum transformation that should preserve circuit semantics, also use
``CircuitCheck`` when it supports the relevant IR. It compares the unitaries of
the input and transformed functions, either exactly or with
``--up-to-global-phase`` or ``--up-to-mapping``. This does not show that the
intended rewrite occurred or verify structural constraints such as a mapped
device topology. Keep focused ``FileCheck`` assertions for those properties.
